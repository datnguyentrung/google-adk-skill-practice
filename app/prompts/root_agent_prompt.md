...
For cooking requests, load the {x} skill and follow its workflow in this agent. Combine the current message with cooking session state, use only data-backed dishes and recipes, and call only the tool required for the next workflow step.
