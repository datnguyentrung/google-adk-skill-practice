"""Phase kiểm định — đối chiếu graph patch với nguồn tài liệu (source grounding).

Module này trả lời câu hỏi: "mọi fact trong patch có thật sự được chống đỡ bởi
chunk nguồn hay không?". Cụ thể:
- kiểm tra coverage: mọi chunk chuẩn bị trước đó phải được khai báo, không thừa/thiếu;
- kiểm tra evidence: chunk tồn tại, đúng source/section và là trích dẫn nguyên văn;
- kiểm tra giá trị thuộc tính và quan hệ giữa các node có được evidence chống đỡ;
- khi so khớp tất định thất bại, chuyển tiếp cho `SemanticValueJudge`/`SemanticJudge`.

Kết quả là danh sách `ValidationIssue` đã khử trùng lặp; module này KHÔNG ghi gì
...
class SourceGroundingValidator:
    ...
    def validate(
        ...
        *,
        ...
        """Kiểm tra một draft patch có được nguồn tài liệu chống đỡ đầy đủ hay không.

        Args:
            draft: Graph patch do LLM trích xuất, chưa compile.
            chunks: Các chunk nguồn đã chuẩn bị cho lần ingestion này.
            context_nodes: Node ngữ cảnh (từ batch trước) để resolve tempId của edge.
            evidence_context: Chunk bổ sung được phép dùng làm evidence (ví dụ
                chunk của batch trước khi trích xuất theo batch).

        Returns:
            Danh sách `ValidationIssue` đã khử trùng lặp, bao gồm lỗi coverage,
            evidence và grounding của property/edge.
        """
        ...
        for position, item in enumerate(draft.coverage):
            # Mỗi chunk chỉ được khai báo coverage một lần.
        ...
        for node_index, node in enumerate(draft.nodes):
            ...
            for property_index, entry in enumerate(node.properties):
                ...
                if grounding == "derived":
                    ...
                    # Thuộc tính do compiler suy ra từ edge thì node phải có ít nhất
                    # một quan hệ đã được grounding.
                    if (
    ...
    def _edge_supported(
        ...
        """Cho biết edge có được evidence chống đỡ hay không (bỏ phần chi tiết).

        Args:
            edge: Edge cần kiểm tra.
            evidence_items: Evidence đã lọc hợp lệ.
            node_by_temp_id: Bảng tra node theo tempId để resolve hai đầu edge.

        Returns:
            `True` nếu judge kết luận `supported`.
        """
    ...
    def _edge_support_details(
        ...
        """Phán xử edge và trả về cả tập chunk đã dùng lẫn chunk không liên quan.

        Args:
            edge: Edge cần phán xử.
            evidence_items: Evidence đã lọc hợp lệ.
            node_by_temp_id: Bảng tra node theo tempId.

        Returns:
            Bộ ba `(supported, grounded_chunks, unrelated_indexes)`. Nếu thiếu
            node hai đầu thì coi như không supported và mọi evidence là thừa.
        """
    ...
    def _property_grounding(
        ...
        """Phân loại cách một thuộc tính cần được grounding.

        Args:
            property_name: Tên kỹ thuật của property.
            attribute: Định nghĩa attribute trong ontology.

        Returns:
            `"derived"` nếu giá trị do runtime/compiler sinh ra, `"normalized"`
            nếu policy yêu cầu so khớp sau chuẩn hoá, còn lại là `"literal"`.
        """
    ...
    def _value_grounding_message(
        ...
        """Dựng message lỗi `PROPERTY_VALUE_NOT_GROUNDED` theo kiểu grounding."""
    ...
    def _validate_evidence(
        ...
        *,
        ...
        """Kiểm tra từng evidence có trỏ đúng chunk nguồn và là trích dẫn nguyên văn.

        Args:
            evidence_items: Evidence cần kiểm tra.
            chunk_by_index: Bảng tra chunk nguồn theo index.
            location_prefix: Tiền tố vị trí dùng cho message lỗi.
            node_temp_id: tempId của node liên quan (để gắn vào issue).
            property_name: Tên property liên quan (để gắn vào issue).
            edge_name: Tên edge liên quan (để gắn vào issue).

        Returns:
            Bộ đôi `(issues, valid_chunks)`; `valid_chunks` chỉ gồm chunk mà
            evidence đã vượt qua mọi kiểm tra.
        """
        ...
        for index, evidence in enumerate(evidence_items):
            ...
            # Trích dẫn phải nằm nguyên văn trong nội dung chunk hoặc trong section.
            if not (
    ...
    def _value_supported(
        ...
        """Cho biết một giá trị đơn có xuất hiện trong text evidence hay không.

        Giá trị list chỉ được coi là supported khi mọi phần tử đều supported.
        Cách so khớp phụ thuộc datatype của `ranges` (date/số/boolean/string).
        """
    ...
    def _value_supported_chunks(
        ...
        """Trả về tập chunk chống đỡ giá trị, thử từng evidence rồi ghép toàn bộ.

        Args:
            value: Giá trị cần kiểm tra (có thể là list nhiều claim).
            evidence_items: Evidence hợp lệ của property.
            ranges: Danh sách xsd range của property.

        Returns:
            Tập chunk index chống đỡ giá trị; rỗng nghĩa là không được chống đỡ.
        """
        ...
        # Một số fact chỉ xuất hiện khi ghép nhiều chunk (ví dụ bảng bị cắt),
        # nên thử thêm phép so khớp trên toàn bộ text gộp.
        combined_text = "\n".join(
    ...
    def _grounding_failure_reason(
        ...
        """Phân loại nguyên nhân grounding thất bại để ghi log chẩn đoán."""
    ...
    def _evidence_support_text(evidence: Evidence) -> str:
        """Ghép section và text của evidence thành text dùng để so khớp."""
    ...
    def _string_supported(cls, value: str, evidence_text: str) -> bool:
        """So khớp chuỗi sau khi chuẩn hoá (NFKC + casefold + gộp khoảng trắng)."""
    ...
    def _number_supported(cls, value: Any, evidence_text: str) -> bool:
        """So khớp số bằng cách bỏ hết ký tự không phải chữ số ở cả hai phía."""
    ...
    def _normalize_number_value(value: float | Decimal) -> str:
        """Chuẩn hoá số về chuỗi chỉ gồm chữ số để so khớp với text nguồn."""
    ...
    def _contains_quote(cls, content: str, quote: str) -> bool:
        """Kiểm tra `quote` có nằm trong `content` (đã chuẩn hoá xuống dòng).

        Với dòng bảng markdown (bắt đầu và kết thúc bằng `|`), yêu cầu khớp cả
        dòng để tránh nhận nhầm một ô của bảng là trích dẫn hợp lệ.
        """
    ...
    def _normalize_line_endings(value: str) -> str:
        """Đưa mọi kiểu xuống dòng (CRLF/CR) về LF trước khi so khớp."""

        return value.replace("\r\n", "\n").replace("\r", "\n")

    @staticmethod
    def _normalize(value: str) -> str:
        """Chuẩn hoá văn bản: NFKC, casefold và gộp khoảng trắng thừa."""
    ...
    def _date_supported(cls, value: Any, source_text: str) -> bool:
        """So khớp ngày theo nhiều định dạng phổ biến (ISO, dd/mm/yyyy, ...)."""
