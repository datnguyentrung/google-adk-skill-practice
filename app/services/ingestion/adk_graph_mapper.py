    ...
    def _prompt(
        ...
        *,
        ...
        return (
            ...
            "- Never emit a persistence-incomplete stub just to serve as an edge endpoint. Before creating any new related entity, verify that every REQUIRED_SOURCE field for that class is groundable now. This is especially important when another product is mentioned only as a related/cross-sell/upsell/substitute/complement reference: if the full BankingProduct source contract is not present and no exact canonical ref exists, do not create a partial BankingProduct node merely to attach the relationship; represent only other source-backed facts that can be persisted safely.\n"
