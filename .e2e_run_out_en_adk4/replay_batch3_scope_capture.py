...
def capture_map_batch(**kwargs):
    ...
    item = {
        ...
        "coverage": [
            {"chunkIndex": c.chunk_index, "decision": c.decision, "reason": c.reason}
            for c in fragment.coverage
        ],
        "documentNames": [
            p.value
            for n in fragment.nodes if n.class_name == "pskg:RequiredDocument"
            for p in n.properties if p.property_name == "pskg:documentName"
        ],
        "nodeClasses": [n.class_name for n in fragment.nodes],
        "fragment": fragment.model_dump(by_alias=True, mode="json", exclude_none=True),
    }
    attempts.append(item)
    print("ATTEMPT", len(attempts), "coverage=", [(c["chunkIndex"], c["decision"]) for c in item["coverage"]], "docs=", item["documentNames"])
    Path('.e2e_run_out_en_adk4/b3_scope_attempts.json').write_text(json.dumps(attempts, ensure_ascii=False, indent=2), encoding='utf-8')
    return fragment
