...
def test_source_literal_value_without_support_is_rejected_for_retry():
    text = "Product code is P-001."
    payload = {
        "nodes": [{
            "tempId": "product-p001",
            "className": "pskg:BankingProduct",
            "properties": [{"propertyName": "pskg:bankingProductName", "value": "Invented Product Name", "evidence": [_evidence(0, text)]}],
            "evidence": [_evidence(0, text)],
            "confidence": 1.0,
        }],
        "edges": [],
    ...
    with pytest.raises(DirectGraphMappingError) as exc:
        mapper.map_batch(batch_payload=_payload([_chunk(content=text)]), chunks=[_chunk(content=text)])
    assert any(item["code"] == "SOURCE_LITERAL_NOT_GROUNDED" for item in exc.value.summary["errors"])
